class CommentUpdateJob < ApplicationJob
  ...
  def perform(comment, current_user)
    # Do something later
    ProductChannel.broadcast_to comment.product_id, comment: CommentsController.render(partial: 'comments/comment', locals: {comment: comment}), average_rating: @product.average_rating
  end
